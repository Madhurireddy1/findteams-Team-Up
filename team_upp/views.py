...
from team_upp.models import Teams, RecruitedTeammates
from team_upp.serializer import ApplicationStatusSerializer
...
def home(request):
    ...
    return render(request, 'team_upp/home.html')
        ...
        if (request.POST['category'] and request.POST['short_description'] and
            ...
            if request.POST['relevant_url']:
                if (request.POST['relevant_url'].startswith('http://') or
                    ...
                    teams.relevant_url = 'https://' + \
                        request.POST['relevant_url']
            else:
            ...
            recruited_teammates.teamup_advertisement = Teams.objects.get(
                id=teams.id)
            recruited_teammates.teammates = Extendeduser.objects.get(
                user=request.user)
            recruited_teammates.save()
            return redirect('/team_upp/' + str(teams.id))
        else:
            return render(request, 'team_upp/create_t_up.html',
                          {'error': 'All astrisk marked (*) fields are required!'})
    else:
        return render(request, 'team_upp/create_t_up.html')


def detail(request, team_upp_id):
    # TODO add a page where all his created t-ups are visible to the user
    teamup = get_object_or_404(Teams, pk=team_upp_id)
    return render(request, 'team_upp/details.html', {'teams': teamup,
                                                     'owner': request.user})
...
def tup_category(request, category):
    ...
    return render(request, 'team_upp/tup_groups.html', {'data': data})
...
def show_teamup_details(request):
    ...
    if(adv_card[0].logged_in_user.user == request.user):
        ...
        owner = 0
    return render(request, 'team_upp/details.html', {'teams': adv_card[0],
...
def join_tup(request, recruiter):
    ...
    return render(request, 'team_upp/join_success.html')
...
def requests(request):
    ...
    if(application_status):
        requester = Extendeduser.objects.get(
            user=application_status[0].requester)
        advertisement = Teams.objects.get(
            id=application_status[0].teamup_advertisement)
        return render(request, 'team_upp/requests.html',
...
def remove_teammate(request, adv):
    ...
    total_page_count = 5
    data = paginate(request, page, user_teams, total_page_count)
    return render(request, 'team_upp/tup_groups.html', {
        'data': data, 'msg': msg})
